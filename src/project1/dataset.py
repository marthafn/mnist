    ...
    def load_data(self) -> None:
        ...
        if self.train:
            ...
            for i in range(nb_files):
                images.append(torch.load(f'{self.data_folder}/train_images_{i}.pt',weights_only=True))
                target.append(torch.load(f'{self.data_folder}/train_target_{i}.pt',weights_only=True))
        else:
            images.append(torch.load(f'{self.data_folder}/test_images.pt',weights_only=True))
            target.append(torch.load(f'{self.data_folder}/test_target.pt',weights_only=True))
